...
export default function VideoPlayer({ videoData }) {
  ...
  useEffect(() => {
    ...
    // HLS setup
    if (Hls.isSupported()) {
      const hls = new Hls({ xhrSetup: (xhr, url) => xhr.open('GET', url, true) });
      hlsRef.current = hls;
      ...
      hls.on(Hls.Events.ERROR, (_, data) => data.fatal && setError(`HLS error: ${data.type}`));
  ...
  const seekBy = (sec) => videoRef.current && (videoRef.current.currentTime += sec);
  const selectTime = (e) => {
  ...
  return (
    <div className="relative group">
      ...
      <div className="absolute left-0 right-0 bottom-0 h-10 flex items-center px-3 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        >
          <div className="absolute top-0 left-0 h-1 bg-red-600" style={{ width: `${progressPct}%` }} />
          {hoverTime !== null && (
            <div style={{ left: hoverX }} className="absolute -top-6 px-1 text-xs bg-black text-white rounded">
        ...
        <div className="ml-4 relative">
          ...
          {showQualityMenu && (
            <div className="absolute bottom-8 right-0 bg-black bg-opacity-75 text-white rounded shadow-lg">
              <button onClick={() => handleQualityChange(-1)} className="block px-3 py-1 w-full text-left hover:bg-gray-700">Auto</button>
              {levels.map((lvl, idx) => (
                <button key={idx} onClick={() => handleQualityChange(idx)} className="block px-3 py-1 w-full text-left hover:bg-gray-700">{lvl.height}p</button>
              ))}
            </div>
          )}
        </div>
